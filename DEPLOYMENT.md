...
- 2026-09-22 已执行 `scripts/bootstrap.py`:管理 API Key 已生成。公开注册最初关闭,同日应用户要求重新开放(sub2api `registration_enabled=true`,Caddy 放行注册接口);防滥用依赖新用户默认 0 余额(`default_balance=0`,无注册赠送),需管理员充值后才能调用。
